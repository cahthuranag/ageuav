...
def build_model(snrdb,blocksize,ric_K):
    ...
    if channel_type == "awgn":
        ...
        z_out, h =fading(z_in, noise_stddev,ric_K)
        # convert back to real
        z_out = tf.concat([tf.math.real(z_out), tf.math.imag(z_out)], 1)
...
def plot_accuracy_vs_snr(snr_values_db, accuracy_results):
    ...
    plt.xlabel('SNR (dB)',font_family='Times New Roman',fontsize=14)
    plt.ylabel('Classficaiton accuracy',font_family='Times New Roman',fontsize=14) #classficaiton accuracy
...
def test_accurcy_block(snr_value_db,x_train, y_train, x_test, y_test, block_size,ric_k):
    sim_num = 1
    sim_acurracy = []
    for _ in range(sim_num):
        classifier_model = build_model(snr_value_db,block_size,ric_k)
        classifier_model.fit(x_train, y_train, epochs=30, batch_size=128, validation_data=(x_test, y_test))
