...
if arrival_timestamps[num_events - 1] < departure_timestamps[num_events - 2]:
     departure_timestamps_new.append( departure_timestamps[num_events - 1])
     departure_timestamps_new.append(arrival_timestamps[num_events - 2])
else:
    if np.random.rand() > erov_s:
        departure_timestamps_new.append(departure_timestamps[num_events - 1])
        departure_timestamps_new.append(arrival_timestamps[num_events - 2])
        ...
        departure_timestamps_new.append(departure_timestamps[num_events - 1])
        departure_timestamps_new.append(arrival_timestamps[num_events - 1])
...
erov = erov_s
age_theory = (1 /  ((1 - erov) * lambda1)) + (1 / ((1 - erov) * mu)) + ((lambda1) / (((mu ) + lambda1) * (mu )))
