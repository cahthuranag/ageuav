...
def main():
    ...
    snr_values_db = np.linspace(-10, 20, num=20)  # SNR values from 0 to 20 dB with 11 steps

    accuracy_results = []
    for snr_value_db in snr_values_db:
        classifier_model = build_autoencoder_classifier(snr_value_db)

        classifier_model.fit(x_train, y_train, epochs=20, batch_size=32, validation_data=(x_test, y_test))
